	...
	switch($valor)
	{
		...
		case "fecha":
		{
			...
			if ($fecha_hasta2 < $fecha_desde2)
			{
				...
				$url_relativa = "siscor/vista/reportes_recibidas.php";
				header("Cache-Control: no-cache");
		...
		case "direccion":
		{
			...
			else
			{
				$fecha_desde=$Recibidas->arregla_fecha($_POST['fecha_desde']);
				$fecha_hasta=$Recibidas->arregla_fecha($_POST['fecha_hasta']);
			$parametro="cd_alto_nivel_corres=".$Recibidas->getAlto_nivel()." and cd_primer_nivel_corres=".$Recibidas->getPrimer_nivel()." and fe_entrada_recibidas BETWEEN to_date('".$fecha_desde."','YYYY.MM.DD') and to_date('".$fecha_hasta."','YYYY.MM.DD')";
			}
			...
			case "respuesta":
		{
			...
			else if($Recibidas->getRespondida()=="NO")
			{
				$Recibidas->setRespondida('false');
			}
			$fecha_desde=$Recibidas->arregla_fecha($_POST['fecha_desde']);
			$fecha_hasta=$Recibidas->arregla_fecha($_POST['fecha_hasta']);
			if($Recibidas->getRespondida()=='TODOS')
			{
				$parametro="fe_entrada_recibidas BETWEEN to_date('".$fecha_desde."','YYYY.MM.DD') and to_date('".$fecha_hasta."','YYYY.MM.DD')";	
