	...
	switch($valor)
	{
		 ...
		 case "fecha":
		 {
			...
			if ($fecha_hasta2 < $fecha_desde2)
			{
				...
				$url_relativa = "siscor/vista/reportes_remisiones_respuesta.php";
				header("Cache-Control: no-cache");
				header("Location: http://" . $_SERVER['HTTP_HOST']  . "/" .$url_relativa);
			}
			else
		...
		case "direccion":
		{
			...
			{
				$parametro="cd_alto_nivel=".$Remisiones->getAlto_nivel()." and cd_primer_nivel=".$Remisiones->getPrimer_nivel()."and fe_remisiones BETWEEN to_date('".$fecha_desde."','YYYY.MM.DD') and to_date('".$fecha_hasta."','YYYY.MM.DD')";
	//haciendo el reporte por remision de la direccion unos tienen direccion y otro no
...
//die($_SESSION['sql']."aaaaaaaaaaaaaaaaaaaaaaa");
	...
	if ( $cont != 0 ) 
	{
    	...
    	$primer_nivel=array();
    	$direcciones=array();
    	$camporespuesta=array();
    	$camporesponsable=array();
    	$campotextrespuesta=array();
		...
		//Carga los registros
			...
			{ 
    	    	...
    	    	array_push( $camporespuesta , $row['in_respuesta_remisiones']);
		    	array_push( $camporesponsable , $row['nb_responsable_remisiones']);
		    	array_push( $campotextrespuesta , $row['tx_respuesta_remisiones']);
			}
    	//Prepara para la comunicacion
    	    	...
    	    	$_SESSION['in_respuesta_remisiones'] = $camporespuesta;
		    	$_SESSION['nb_responsable_remisiones'] = $camporesponsable;
		    	$_SESSION['tx_respuesta_remisiones'] = $campotextrespuesta;            	
