...
include_once("../modelo/class_modulosxusuarios.php"); 
	...
	$permiso =$Modulosxusuarios->Existe($Oficios->getId_usuario(),'in_ingresar',"TRUE",5);
...
if ($_SESSION['perfil']==1 && $permiso==1)
{
   	...
   	$RespuestaOficio = isset($_GET['Resp_ofi'])? $_GET['Resp_ofi'] : "respuesta";

   	
   	if ($RespuestaOficio=="cons")
   	{
   		$_SESSION['ConsultarResp']=1;
   	}
...
if($_POST['consultar']==true)
{
	...
	if($Resp == False)
	{
		...
		$url_relativa = "siscor/vista/registrar_respuesta_oficios.php";
	...
	if ($_SESSION['RespuestaOfic']!=1 )
	{
		...
		else
		{
			...
			if ($_SESSION['ConsultarRespuesta']==1)
			{
				...
				$_SESSION['error_recibidas']="Este N&uacute;mero de Oficio no se le ha asignado una Respuesta";
				$url_relativa = "siscor/vista/registrar_respuesta_oficios.php?Resp_ofi=cons";	
				header("Cache-Control: no-cache");
				header("Location: http://" . $_SERVER['HTTP_HOST']  . "/" .$url_relativa);	
				exit;					
	...
	$_SESSION['fecha_envio_oficios']=$Oficios->getFeenvio();
...
if($_POST['cancelar_oficios']==true)
{
