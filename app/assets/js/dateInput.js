/*****************************************************************************
Copyright (C) 2006  Consultores Micorp

Based on the Epoch package released by Nick Baicoianu and modified by
Consultores Micorp to add the preset current date feature in popup mode.
Other changes were also made, such as in popup mode being contained in the
targetelement rather than document and support for positioning within
scrolled containers.
...
function Epoch(name,mode,targetelement,removable,multiselect)
{
  this.state = 0;
  this.name = name;
  ...
  this.selectMultiple = (multiselect == true); //'false' is not true or not set at all
	this.setButton = !(removable == false);
  // initialize curDate using a value in the container input.  If empty or unparsable, curDate is unchanged from default init value.
  if(this.mode == 'popup' && targetelement && targetelement.type == 'text') //if the target element has been set to be an input text box
  {
    this.curDate.dateParse(targetelement.value);
    targetelement.title = 'Presione para ver el Calendario';
  }
  ...
  if(this.mode == 'popup' && targetelement && targetelement.type == 'text') //if the target element has been set to be an input text box
  {
    ...
    this.calendar.style.top = 20 + this.getTop(targetelement) + this.topOffset + 'px';
    this.calendar.style.left = this.getLeft(targetelement) - this.getScrollLeft(targetelement) + this.leftOffset + 'px';
    //document.body.appendChild(this.calendar);
    this.tgt.parentNode.appendChild(this.calendar);
...
{
  this.daylist = new Array('Do','Lu','Ma','Mi','Ju','Vi','Sa'); /*<lang:es>*/
  ...
  this.clearbtn_caption = 'Borrar';
  this.clearbtn_title = 'Borra la fecha seleccionado';
...
//-----------------------------------------------------------------------------
Epoch.prototype.getScrollLeft = function (element) //PRIVATE: returns the total scrolling of all parent containers
{
    var oNode = element;
    var iScrollLeft = 0;

    while(oNode.tagName != 'BODY') {
        iScrollLeft += oNode.scrollLeft;
...
Epoch.prototype.show = function () //PUBLIC: displays the calendar
{
  ...
  if ( this.mode == 'popup' )
  {
    this.calendar.style.top =  20 + this.getTop(this.tgt) + this.topOffset + 'px';
    this.calendar.style.left = this.getLeft(this.tgt) - this.getScrollLeft(this.tgt) + this.leftOffset + 'px';
  }
};
//-----------------------------------------------------------------------------
...
{
  this.calendar.style.top =  20 + this.getTop(target) + this.topOffset + 'px'
...
//-----------------------------------------------------------------------------
Date.prototype.dateParse = function(dateString)
{
  /* Array of objects, each has 're', a regular expression and 'handler', a
     function for creating a date from something that matches the regular
     expression.  Earlier entries are matched first.
  */
