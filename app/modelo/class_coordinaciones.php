...
/**
	 ...
	 */
	/**
	 * @return the $cd_unidades
	 */
	public function getCd_unidades() {
		return $this->cd_unidades;
	}

/**
	 * @return the $nombre_unidades
	 */
	public function getNombre_unidades() {
		return $this->nombre_unidades;
	}

/**
	 * @param $cd_unidades the $cd_unidades to set
	 */
	public function setCd_unidades($cd_unidades) {
		$this->cd_unidades = $cd_unidades;
	}

/**
	 * @param $nombre_unidades the $nombre_unidades to set
	 */
	public function setNombre_unidades($nombre_unidades) {
			...
			else 
			{
				...
				if (pg_num_rows ($Resultado) == 0) 
                       ...
                       else 
                       {
                            ...
                            while ( $row = pg_fetch_array ( $Resultado ) ) 
                            {
                                       $salida .= "<tr class='modo1'><td class='font2'>" . $row ["nb_coordinaciones"] . "</td><td class='font2' align='center'><a href='../controlador/control_coordinaciones.php?form=mod&id=" . $row ["cd_coordinaciones"] . "'><img src='../assets/img/edit.png'  border='0' title='Modificar' ></a>
                                       <a href='../controlador/control_coordinaciones.php?form=eli&id=" . $row ["cd_coordinaciones"] . "' onClick='return confirma();'>
 ...
 function CargarDatos()
   {  
   	$codigo = $this->getId();
   	  	...
   	  	$BaseDato=new BaseDatos();//declarar el objeto de la clase base de dato
   	  	$sql = "SELECT * FROM vista_mostrar_coordinaciones WHERE cd_coordinaciones='$codigo'";
   	  	...
   	  	{
   	  		$this->setNombre($resultado->fields['nb_coordinaciones']) ;
   	  		...
   	  		$this->setCd_unidades($resultado->fields['cd_unidades']);
   	  		$this->setUbicacion($resultado->fields['ds_ubicacion_coordinaciones']);
   	  		$this->setEdificio($resultado->fields['ds_edificio_coordinaciones']); 
   	  	    $this->setPiso($resultado->fields['ds_piso_coordinaciones']);
   	  		$this->setTelefono($resultado->fields['ds_telefono_coordinaciones']);
   	  		$this->setObservacion($resultado->fields['tx_observacion_coordinaciones']);
   	  		$this->setNombre_primer_nivel($resultado->fields['nb_primer_nivel']);
   	  		$this->setNombre_direcciones($resultado->fields['nb_direcciones']);
   	  		$this->setNombre_unidades($resultado->fields['nb_unidades']);
