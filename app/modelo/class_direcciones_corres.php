...
function __construct(/*$login,$contrasena,$cedula,$tipo,$status*/)
   ...
   {            
		...
		return $BaseDato->EjecutarVista("vista_mostrar_alto_nivel_corres");
      //return 1;
			...
			else 
			{
				...
				if (pg_num_rows ($Resultado) == 0) 
                       ...
                       else 
                       {
                            ...
                            while ( $row = pg_fetch_array ( $Resultado ) ) 
                            {
                                       $salida .= "<tr class='modo1'><td class='font2'>" . $row ["nb_direcciones_corres"] . "</td><td class='font2' align='center'><a href='../controlador/control_direcciones_corres.php?form=mod&id=" . $row ["cd_direcciones_corres"] . "&idd=".$row['cd_direcciones_usuarios']."'><img src='../assets/img/edit.png'  border='0' title='Modificar' ></a>
                                       <a href='../controlador/control_direcciones_corres.php?form=eli&id=" . $row ["cd_direcciones_corres"] . "&idd=".$row['cd_direcciones_usuarios']."' onClick='return confirma();'>
 ...
 function CargarDatos()
   {  
   	$codigo = $this->getId();
   	  	...
   	  	$BaseDato=new BaseDatos();//declarar el objeto de la clase base de dato
   	  	$sql = "SELECT * FROM vista_mostrar_direcciones_corres WHERE cd_direcciones_corres='$codigo'";
   	  	$resultado=$BaseDato->EjecutarQuery($sql);
   	  	if ($resultado->fields['nb_direcciones_corres']=="")
   	  	{
   	  		return FALSE;
   	  	}
   	  	else 
   	  	{
   	  		$this->setNombre($resultado->fields['nb_direcciones_corres']) ;
   	  		$this->setCd_alto_nivel($resultado->fields['cd_alto_nivel_corres']);
   	  		$this->setCd_primer_nivel($resultado->fields['cd_primer_nivel_corres']);
   	  		$this->setUbicacion($resultado->fields['ds_ubicacion_direcciones_corres']);
   	  		$this->setEdificio($resultado->fields['ds_edificio_direcciones_corres']); 
   	  	    $this->setPiso($resultado->fields['ds_piso_direcciones_corres']);
   	  		$this->setTelefono($resultado->fields['ds_telefono_direcciones_corres']);
   	  		$this->setObservacion($resultado->fields['tx_observacion_direcciones_corres']);
   	  		$this->setNombre_primer_nivel($resultado->fields['nb_primer_nivel_corres']);
